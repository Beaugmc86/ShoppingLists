...
import { StyleSheet, View, FlatList, Platform, Text, TextInput, KeyboardAvoidingView, TouchableOpacity, Alert, unstable_batchedUpdates } from 'react-native';
import { collection, getDocs, addDoc, onSnapshot, query, where } from "firebase/firestore";

const ShoppingLists = ({ db, route }) => {
  const { userID } = route.params;

  // States that will hold the list(s) fetched from the database.
  const [lists, setLists] = useState([]);
  const [listName, setListName] = useState("");
  const [item1, setItem1] = useState("");
  const [item2, setItem2] = useState("");

  // Add data query to add items to shopping list
